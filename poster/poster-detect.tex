...
\usepackage[top=2.2cm,right=2.8cm,bottom=2.8cm,left=2.6cm]{geometry}
...
%\vspace{1cm}
\veryHuge \color{NavyBlue} \textbf{Quantum Time: Page-Wootters and Detector\\[0.5cm] Models [\emph{draft}]} \color{Black}\\[0.5cm] % Title
...
\Large University College Cork, Department of Physics\\[0.4cm] % University/organization
\large%
\texttt{gderosa@umail.ucc.ie}
\end{minipage}%
%
\begin{minipage}[t]{0.25\linewidth}
\hspace{3cm}\includegraphics[width=16cm]{ucc_logo.pdf}\\  %% vector, from original SVG
\end{minipage}
...
in a finite-dimensional system, is obtained via Discrete Fourier Transofmration
\cite{FiniteHilb}.
...
can be formlalized in a compact way for each eigenvector of $\hat{\mathbb{J}}$:
\begin{equation}
...
the distortion caused by the detector is negligeable.
...
in lack of a unversally accepted definition of a time-of-arrival observable).
...
\begin{center}
  ...
  \captionof{figure}{
    \color{Green}
    Loss of normalization (small but noticeable on top-right) and components of the qutrit over the first cycle $t \in (0, \sqrt{2}\pi)$.
  }
\end{center}
\vspace{0.5cm}
\vspace{0.5cm}
...
\begin{center}
  ...
  \captionof{figure}{
    \color{Green}
    Time-of-arrival at $ket{2}$ per (discrete) PaW model,
    using conditional definition of \cite{Maccone:QMOT} and no complex potential~(points).
    Detector model with complex potential, and probabilty conditioned in the first cycle (line).
  }
\end{center}

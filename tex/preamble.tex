\documentclass{book}

\usepackage{xcolor}
\usepackage{lipsum}  %% dev
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{amsthm}
\usepackage{xparse}  %% required by physics
\usepackage{physics}
\usepackage{breqn}
\usepackage{bbm}
\usepackage[utf8]{inputenc}
\usepackage{mathtools}
\usepackage[super]{nth}
\usepackage[inline]{enumitem}
\usepackage{graphicx}
\usepackage{caption} % https://tex.stackexchange.com/a/43996
\usepackage{color}
% \ifscr
%   \usepackage[b5paper,top=1.5cm,bottom=1.5cm,left=0.65cm,right=0.65cm,marginpar=0cm]{geometry}
% \else
  \usepackage[a4paper,marginpar=0cm]{geometry}
% \fi
\usepackage{fancyhdr}
\pagestyle{fancy}  %% prevent long section titles from overflowing page headers
  \fancyhf{}
  \fancyhead[LE]{\thepage}
  \fancyhead[RE]{\textit{\nouppercase{\leftmark}}}
  \fancyhead[LO]{\textit{\nouppercase{\rightmark}}}
  \fancyhead[RO]{\thepage}
\usepackage[safeinputenc,citestyle=authoryear-icomp,bibstyle=authoryear,backend=biber]{biblatex}
\usepackage{environ}
\usepackage{epigraph}
\usepackage{csquotes}
  \setlength{\epigraphwidth}{.6\textwidth}
\usepackage{fancyvrb}
  \fvset{
    frame = single,
    rulecolor=\color{lightgray}
  }
\usepackage{listings}
  \lstset{
    frame = single,
    rulecolor=\color{lightgray},
    basicstyle=\footnotesize\ttfamily,
    commentstyle=\color{gray},
    keywordstyle=\color{blue},
    stringstyle=\color{red},
    fancyvrb=true,
    breaklines=true,
    %identifierstyle=\color{green!40!black}
  }
\usepackage{setspace}
\usepackage[  %% be the last -- https://tex.stackexchange.com/a/326953
  colorlinks,
  citecolor=green!75!black,
  linkcolor=red!75!black,
  urlcolor=magenta!75!black
]{hyperref}
% https://tex.stackexchange.com/a/65547
\usepackage{bookmark}


\input{tex/lib}

\renewcommand{\arraystretch}{1.6}

\author{Guido De Rosa \\ \small\tt{gderosa@umail.ucc.ie}}
\title{It's $\ket{1}$ o'clock -- Relational Time and Applications}

\addbibresource{bib/main.bib}
\addbibresource{bib/comp.bib}

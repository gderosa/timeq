...
We derive the components of $\dket{\Phi}$ numerically in \S \ref{detection-event},
...
at any time $t$
(%
  which is not surprising, being $\hat{D}^{1/2}$ of the form
  $\footnotesize \left[\begin{matrix}0 & 0\\0 & \kappa \end{matrix}\right]$%
).
In more formal terms:
\begin{equation}\label{eq:detect_prob_amplitude_proportional}
  {}_T\!\bra{t} {}_S\!\bradket{1}{\Phi} \;\propto\; {}_T\!\bra{t} {}_S\!\bradket{1}{\Psi} \; \forall t \text{,}
\end{equation}
which, algbebraically, is almost obvious, given the considerations above.
However, it's worth noting that the expression on the right side, at each $t$,
is an ordinary quantum mechanical probability amplitude
(probability of being $\ket{1}$ rather than $\ket{0}$),
while the expression on the left side, as a function of $t$,
is a probability (amplitude) distribution \emph{in time}.
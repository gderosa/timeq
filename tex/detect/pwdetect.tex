...
${}_T\bradket{0}{\Psi} = \sum_n \alpha_n \, {}_T\bradket{0}{n} = \ket{0}_S$.

The vector $\dket{\Psi}$, proper element of $\pwspace$,
is the Page--Wootters counterpart of the function~$\hat{\psi}$
(``in time representation'') as
described in \cite{RuschhauptAbsorption}.

As a consequence of what observed while commenting the \eqref{eq:analytic:hatpsi},
the ``detection wavefunction''~$\dket{\Phi}$ will always lie,
\emph{spatially},
in the space generated by $\ket{1}_S$,
which is the ``detectable area''
within the problem at hand:
$\dket{\Phi}$ is essentially the result of the action of the operator $\hat{D}$,
that filters out non-detectable components in space from any vector of $\hilb{H}_S$:
\begin{equation}
  \bra{t}\bradket{0}{\Phi} = 0 \text{,} \quad \forall t \, \text{.}
\end{equation}

...
\begin{align}
  ...
  V(x)    &= \theta(-x) \,\text{,}
\end{align}
with $a$, $a^{\dagger}$ being the creation and annihilation operator for the electromagnetic field,
which constitutes the ``environment''. $V(x)$ is chosen to be a step function, the simplest that makes the detector only sensitive
in a half line ($x<0$). The Hamiltonian $H_s$ is that of a free particle in this model.
$\kappa$ is a coupling constant (in the same sense of the Jaynes--Cummings model, which incidentally also $H_d$ and $H_s$ recall). 

TODO cite \cite{Damborenea}, or better \cite{Damborenea_atomic}.

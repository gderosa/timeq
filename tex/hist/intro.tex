...
The relation between time and other observable quantities
(whether they are ``quantum observables'' in the formal sense or not)
is explored in what follows,
...
\subsection{Is time ``special''? and does it matter?}
...
which is not \emph{directly} related to the special status of time
as independent variable of ``evolution'':
it is more directly related to special properties of the Hamiltonian,
in particular of its spectrum, as illustrated in Section~\ref{proof}.
...
the one between position and momentum, $[x, p] = \iu \hbar$,
and a consequent
time--energy uncertainty relation.

Incidentally, it's important to stress that, while equations \eqref{eq:genspacetransl} and \eqref{eq:gentimetransl}
do offer a conceptual motivation towards defining a time observable that is conjugate to the Hamiltonian,
they are not sufficient to logically imply its existence: strictly speaking, Eq. \eqref{eq:genspacetransl} does not even
prove that an Hermitian \emph{position} operator exists either.
Both $\Delta{x}$ and $\Delta{t}$ ---which quantifies the spatial and temporal translations at the core of this discussion---
are simply, once again, exact \emph{parameters} for the displacement and time evolution operators respectively.
There is no statistical distribution nor the possibility
of a coherent superposition among different possible values of them.

\subsubsection{Time--energy uncertainty relations}

\cite{Heisenberg:Uncertainty}. (1.1.3 in the Book TQM1). time-frequency signals / telcos. Later we do it.

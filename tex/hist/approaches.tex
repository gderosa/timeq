...
In general, probability distributions are given by bilinear functionals of the wavefunction.
Therefore, the first requirement is:
\begin{enumerate}
  \item The time-of-arrival distribution is given by a continuous positive bilinear functional $F[\psi_{\bar{t}}]$.
\end{enumerate}
...
\begin{enumerate}[resume]
  \item $F[\phi_t] \ge 0 \; \forall t$
  \item If $\norm{\phi} = 1$, then $\int F[\phi_t] \dd t = 1$
  \item $F[\pi(\phi)] = F[\phi]$.
\end{enumerate}
In the last one, $\pi$
...
``traveling upwards from the bottom'' to be equivalent to
to a particle
...
\begin{enumerate}[resume]
  \item $F[\phi^{*}_t] = F[\phi_t]$, $\forall t$.
\end{enumerate}
...
\begin{theorem}
  ...
  \begin{enumerate}
    ...
    \item
      ...
      \begin{equation}
        F_0[\phi_t] = 
            \frac{1}{m(2\pi\hbar)^4} \int_{p_{3}', p_{3}'' \ge 0} \sqrt{p_{3}' p_{3}''} \phi_t^{*}(p_1, p_2, p_{3}') \phi_t(p_1, p_2, p_{3}'')

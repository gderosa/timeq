...
% \end{lstlisting}
\begin{equation}
  \Delta{H_T} = \delta{H_T} = \hbar\qty|\omega_0 - \omega_1|
\end{equation}

\subsubsection*{$\op{H}_T = \hbar\op{\Omega}$ in its eigenbasis}

Diagonal matrix of the eigenvalues of $\op{H}_T$:
\begin{equation}\label{eq:pw:moreva:H_T_diag}
  H_T \repr[H_T] \mqty(\dmat{-\hbar\omega, \hbar\omega})
\end{equation}
...
\begin{equation}
  \op{T} \repr[H_T] \frac{\pi}{(\delta{H_T})^2} F \mqty(\dmat{-\hbar\omega, \hbar\omega}) F^{\dagger}
  =

...
\begin{figure}[h]
  ...
  \begin{subfigure}[b]{\textwidth}
    ...
    \subcaption{
      \emph{Stacked} probabilities $\abs{\braket{i}{\psi(t)}}^2_{i = 0, 1, 2}$
      visualize the fact that their sum
      ---which is equal to $\norm{\psi(t)}^2$---
      is constantly $1$.
      In other words, the norm is conserved,
      as
...
We are also interested in the probability \term{amplitudes}:
complex values of
$\braket{0}{\psi(t)}$, 
$\braket{1}{\psi(t)}$, and
$\braket{2}{\psi(t)}$
are plotted in Fig. \ref{fig:3lev:hermitianEvol}.
Once again, the two horizontal axes are used for real and imaginary part of their values,
...
\begin{figure}[h]
  ...
  \caption{
    ...
    The horizontal plane is the complex plane of their values, with real and imaginary part on the two ases.
    The vertical axis represents time $t$ (independent variable).

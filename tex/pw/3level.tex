...
\end{lstlisting}
where the Python function \lstinline{K()} is the non-Hermitian operator $K$ of eq.~\eqref{eq:3lev:nonUnitaryH}.
...
As the Wheeler-DeWitt reads $\mathbb{J} \dket{\Psi} = 0$,
eigenvectors of $\mathbb{J}$ related to eigenvalues, say $\epsilon_{n}$,
which are different than zero,
need to be corrected by ``rigidly shifting the spectrum of $H_{S}$
by [$\epsilon_{n}$]'' \parencite[``\textit{The Zero-eigenvalue}'']{Lloyd:Time}:
\begin{equation}
...
TODO: ``see also'' eq.~\eqref{eq:pw-vs-qm} and Sec.~\ref{sec:qubit:pw-vs-qm}.

...
eigenvectors of $\mathbb{J}$ related to eigenvalues ---say, $\epsilon_{n}$---
which are not zero,
need to be corrected by ``rigidly shifting the spectrum of $H_{S}$
by [$\epsilon_{n}$]'' (once again, \cite[``\textit{The Zero-eigenvalue}'']{Lloyd:Time}):
\begin{equation}
  \dket{\epsilon_n} \;\longrightarrow\; \dket{\Phi_{n}} \eqbydef e^{ -\iu \hat{T} \epsilon_{n} \, \ox \, \idop_{S}} \dket{\epsilon_{n}} \text{,}
  \label{eq:3lev:correction}
\end{equation}
where it has been set $\hbar=1$. This translates in code as follows.
\begin{lstlisting}[language=Python]
...
the variable \verb|histories[ ]|, in the above code snippet,
corresponds to the $\dket{\Phi_n}$ in Eq.~\eqref{eq:3lev:correction}.
It's named \verb!histories[ ]! because each element
(and any linear combination, per quantum superposition principle)
encodes a whole possible evolution (altough with discrete time resolution)
of the system along the time interval $(0, \Delta T)$.

...
eigenvectors of $\mathbb{J}$ related to eigenvalues $\epsilon$
other than zero
need to be corrected by ``rigidly shifting the spectrum of $H_{S}$
by $\epsilon$'' \parencite[``\textit{The Zero-eigenvalue}'']{Lloyd:Time}:
\begin{equation}
  \dket{\epsilon} \; \longrightarrow \; e^{ -\iu \hat{T} \epsilon \, \ox \, \idop_{S}} \dket{\epsilon} \text{,}
\end{equation}

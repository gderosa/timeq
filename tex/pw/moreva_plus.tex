...
[TODO? : short intro to stress this is mostly original work, and rationale for it]

\subsection{Time operator, diagonalization, evolution}
\label{1qubitExp}

The experiment described in \cite{Moreva:illustration},
in its essence,
is based on two qubits,
physically realized through the polarization states
(vertical or horizontal) of two photons.
Let the first one be the ``clock'' and the second one the ``rest of the universe''
in the Page--Wootters model sense.
The notation $\ket{H}_T, \ket{V}_T, \ket{H}_S, \ket{V}_S$
will be used to refer to the horizontal and vertical polarization states of the two photons respectively.
Consistently with the model, the experiment is prepared in such way
to show that the overall system 
is stationary (i.e. it is an eigenstate of the overall Hamiltonian)
...
defined explicitly by eq. \eqref{eq:MorevaOmegaT}.
Let us recall that it is $\hbar\op{\Omega} = \op{H}_T$, i.e. the ``Hamiltonian of the clock''.

With respect to the polarization basis
$\qty{\ket{H}_T, \ket{V}_T}$ the operator $\op{H}_T$ is then represented in matrix form as
\begin{equation}
...
\end{equation}
The Hamiltonian in $\hilb{H}_S$, in the same experiment, is chosen as
...
for the matrices of $\op{H}_S$ and $\op{H}_T$ to be formally identical.
They represent operators acting on two different Hilbert spaces.
In another experiment, they may even act on spaces of different dimensionality.


The spectrum of $\op{\Omega}$ is $\qty{-\omega, \omega}$.

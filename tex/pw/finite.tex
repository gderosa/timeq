...
\pagebreak  % remove if no longer necessary...
If we introduce the \term{Fourier operator}
$\displaystyle \op{F} \eqbydef \sum_{m,n=0}^{N-1} F_{mn} \ketbra{t_m}{t_n}$,
eq.~\eqref{eq:DFT:bra} can be reformulated\footnote{
  From \eqref{eq:DFT:bra} ---and, similarly, \eqref{eq:DFT:ket}--- multiplying each element of the sum
  by $\braket{t_m} = 1$ we get
  $\displaystyle \bra{\omega_{m}} = \bra{t_m} \sum_{n} F_{mn} \ketbra{t_m}{t_n}$.
}
as
\begin{equation}
...
% ???
% \footnote{
%   \cite[pp. 469, 490]{Carlson}.
% }
(in fact, the paper defines ``position'' $\op{X}$ and ``momentum'' $\op{P}$,
but in a similar fashion).
% In other words, the sampling interval and the fundamental harmonic angular frequency
% are both rescaled to $1$.
...
(see, for example, \cite[Sec. 11.5]{Carlson}).

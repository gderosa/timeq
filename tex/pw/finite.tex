...
would depend on the particular state vector $\ket{\psi} \in \hilb{H}_d$,
and one should compute, explicitly, the minimum of such value over $\ket{\psi} \in \hilb{H}_d$
...
(of the two discrete operators)
with the value predicted by the continuous theory.

% Nevertheless, if the finite-dimensional system of interest is simply the discrete
% approximation

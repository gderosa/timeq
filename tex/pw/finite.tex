...
This means
measuring time in units of \term{number of samples} ($t_1 = \delta{t}$);
and anglar frequency in units of \term{cycles / sampling period}
($\omega_{1} = \frac{2\pi}{\Delta{T}}$).
% ???
% \footnote{
%   \cite[pp. 469, 490]{Carlson}.
% }
In actual fact, the paper defines ``position'' $\op{X}$ and ``momentum'' $\op{P}$,

...
in the overall space $\pwspace$. The advantage of this approach is that
it gives a formal proof related to the spread of the probability distributions
related to measurements
of $\op{T}\ox\idop_S$ and $\hbar\op{\Omega}\ox\idop_S$ on a pure state of $\pwspace$.
The important limitation though is that there is no operational
definition of how to measure such observables \emph{on the whole $\pwspace$},
which would imply a notion of ``external time'',
somewhat contradicting the ``timeless universe'' on which the model is based.
...
(related, say, to eigenvalue $E_0 \eqbydef -\hbar\omega_{0}$): in this case the evolution is obviously
$\ket{\psi(t)}_{S} = \E^{\iu\hbar\omega_{0}(t-t_{0})} \ket{\psi(t_0)}_{S}$
and the measure $\mu$ in eq. \eqref{eq:pw:time_freq_mu}
is expected to only ``select'' $\omega_0$ among the possible values
of $\omega$.

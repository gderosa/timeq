...
\begin{equation}
  \dket{\Psi} = \int d\omega \, \mu(\omega) \ket{\omega}_{T}\ox\ket{\phi(\omega)}_{S} = \int dt \, \nu(t) \ket{t}_{T} \ox \ket{\psi(t)}_{S}\,\text{,} 
\end{equation}
where linear coefficients $\mu$ and $\nu$ must guarantee the normalization of $\dket{\Psi}$ in $\pwspace$.

...
For example, an extreme case is the evolution of an energy eigenstate
(related, say, to eigenvalue $\hbar\omega_{0}$): in this case the evolution is obviously
$\ket{\psi(t)}_{S} = \E^{-\iu\hbar\omega_{0}(t-t_{0})} \ket{\psi(t_0)}_{S}$
and the measure $\mu$ is $\mu(\omega) = \delta(\omega-\omega_{0})$.

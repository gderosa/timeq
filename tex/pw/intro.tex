...
the evolution of the ``S'' system will only depend on ``its own''
...
there is an observable $T$ (in $\hilb{H}_T$)
that can be used to
...
% the time operator $T$ is defined in system $A$ ---and canonically conjugate to $H_T$, not $H_S$.
...
A slightly different notation than $\hilb{H}_T$ and $\hilb{H}_S$ (and their tensor product)

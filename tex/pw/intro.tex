...
TODO: redundant or remove following. Under this time representation (or $T$ representation), the operator $\hbar\Omega$,
...
TODO: redundant or remove previous similar.
Canonical commutation relation should hold and can be easily verified
between $\op{T}$ and $\op{\Omega}$
i.e.
$ \displaystyle
  [\op{T}, \op{\Omega}] \repr_{T} [t, -i\pdv{t}] = i \text{,}
$
similarly to $\displaystyle \comm{\op{x}}{\op{p}} \repr_{x} \comm{x}{-i\hbar\pdv{x}} = i\hbar$.
Thus, $\hbar\op{\Omega}$ can be seen as the ``linear momentum''
...
\begin{tcolorbox}[]%
  Here the measure $\mu(\omega)$ was removed from the first integral, as requested.
  However, it needs to be specified that, while $\norm{\psi(t)}_S = 1 \ \forall t \in \mathbb{R}$
  (unitary evolution, consistent to standard QM),
  the same is not guaranteed for $\ket{\phi(\omega)}_S$.
  Particularly, there are values of $\omega$ such that $\hbar\omega$ does not belong to the energy spectrum of the system
  (i.e. the spectrum of $\op{H}_S$):
  in such cases, it must be $\ket{\phi(\omega)}_S = 0$.
  ---See \cite[eq. 10 and following short discussion]{Lloyd:Time}, for a rationale behind $\dd{\mu(\omega)}$.
  
  In other words, we can remove $\mu(\omega)$,
  or write $\dd{\omega}$ instead of $\dd{\mu(\omega)}$ etc.,
  but that role must be ``absorbed'' by $\ket{\phi(\omega)}_S$.
\end{tcolorbox}

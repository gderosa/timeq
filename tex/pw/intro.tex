...
\begin{tcolorbox}
  ...
  Here the measure $\dd{\mu(\omega)}$ was removed from the first integral, as suggested,
  and replaced simply by $\dd{\omega}$.
  
  However, it needs to be specified that, while $\norm{\psi(t)}_S = 1 \ \forall t \in \mathbb{R}$ \linebreak
  (unitary evolution, as in standard QM),
  the same is not guaranteed for $\ket{\phi(\omega)}_S$.
  For example, there are values of $\omega$ such that $\hbar\omega$ does not belong to the energy spectrum of the system
  ...
  Essentially,
  $\dd{\mu(\omega)}$
  is ``absorbed'' in $\ket{\phi(\omega)}_S$.
\end{tcolorbox}

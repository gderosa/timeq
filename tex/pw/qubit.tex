...
transformation as seen in \eqref{eq:SI_Fourier:Omega}. With a large $N$,
...
Each eigenvector encodes the whole history (at discrete times)
...
\begin{align}
  \dbraket{2k+1}{\Phi_{41}} e^{-i \epsilon_{41} t_k} &\sim \braket{0}{\psi(t_k)}_{\mathrm{S}} \\
  \dbraket{2k+2}{\Phi_{41}} e^{-i \epsilon_{41} t_k} &\sim \braket{1}{\psi(t_k)}_{\mathrm{S}}
  \,\text{,}
\end{align}
where the phase $e^{-i \epsilon_{41} t_k}$ is motivated in \cite[``The Zero-eigenvalue'']{Lloyd:Time},
and inessential if one is merely interest in determining and comparing probabilities
$\qty|\braket{0,1}{\psi}|^2$. Please note the double angle bracket notation for vectors
and operations in the tensor product space $\hilb{H}_T \ox \hilb{H}_S$.
...
\begin{figure}
  ...
  \caption{P-W vs Schr{\"o}dinger evolution for $\hat{\mathbb{J}}$ eigenvalue $=11$}
\end{figure}
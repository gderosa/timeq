...
is the only Hilber space under consideration.

For each value of $t \in \mathbb{R}$ there is
\begin{equation}
  \ket{\psi(t)} = \sum_{k=H,V}\ket{k}\braket{k}{\psi(t)} = e^{-i\hat{H}_{S}(t-t_0)/\hbar}\ket{\psi(t_0)}
\end{equation}

Let's pick, as an example the first eigenstate in \eqref{eq:moreva:eigenJ0}.

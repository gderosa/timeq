...
\begin{remark}
  ...
  therefore allowing the experiment to be run on a quantum computer.
...
Appendix \ref{appendix:n-level} contains the numeric computation details
of an $N=32$-level clock, ``timing'' a qubit subject to the same hamiltonian
of the experiment we have seen with eq.~\eqref{eq:H_S}.

We work in ``natural units'' $\hbar = \omega = 1$, where $\omega$ is
intended in the sense of characteristic frequency in the
two-level clock experiment os \S\ref{1qubitExp}.

The clock is built as having an operator $\hat{T}$ which is diagonal in the
\term{computational basis}.
\[
  ...
  \end{pmatrix} \,\text{.}
\]
With this choice, the clock spans the characteristic period $\Delta T = 2\pi$.

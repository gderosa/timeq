...
It holds\footnote{
  ...
  if $ \hat{p} = \hat{F} x \hat{F}^{\dagger} $,
  the correct inverse relation is
  $ x = \hat{F}^{\dagger} p \hat{F}$ and not $ -x = \hat{F} p \hat{F}^{\dagger} $.
} \parencite{FiniteHilb}:
\begin{gather}\label{eq:FourierCanonicalRelations}
  \hat{\Omega} = \hat{F} \hat{T} \hat{F}^{\dagger}\text{;} \quad
  \hat{T} = \hat{F}^{\dagger} \hat{\Omega} \hat{F}
\end{gather}
where, in the ``position'' (or \emph{time}) finite eigenbasis,
\begin{equation}
  \hat{F} = \frac{1}{\sqrt{N}} \sum_{m,n=0}^{N-1} \exp[i\frac{2\pi mn}{N}] \ketbra{m}{n} \, \text{,}
\end{equation}
while in the frequency eigenbasis
\begin{equation}
  \hat{F}^{\dagger} = \frac{1}{\sqrt{N}} \sum_{\mu,\nu=0}^{N-1} \exp[-i\frac{2\pi \mu\nu}{N}] \ketbra{\mu}{\nu} \, \text{,}
\end{equation}
...
\begin{gather}\label{eq:SI_Fourier}
  \hat{\Omega} = \frac{2\pi}{N(\delta T)^2} \hat{F} \hat{T} \hat{F}^{\dagger} = \frac{2\pi N}{\qty(\Delta T)^2} \hat{F} \hat{T} \hat{F}^{\dagger} \\
  \hat{T} = \frac{2\pi}{N(\delta\Omega)^2} \hat{F}^{\dagger} \hat{\Omega} \hat{F} = \frac{2\pi N}{\qty(\Delta\Omega)^2} \hat{F}^{\dagger} \hat{\Omega} \hat{F}
  \, \text{,}
\end{gather}
where $\delta T$ (and analogously $\delta\Omega$)
is the size of a ``temporal sample'', or the size of a discrete
time step in the clock e.g. $\delta T = \text{1 hour}$ and $N=12$
for a common clock (hours hand) in our everyday life.
Alternatively, e.g. for time, $\Delta T = N \delta T$ is the measurement range of the clock,
and the second notation can be used.
It holds
\begin{equation}
  \delta\Omega \delta T = \frac{2\pi}{N} \, \text{.}
\end{equation}
...
\end{equation}
By using the \eqref{eq:FourierCanonicalRelations}  we can derive the time operator matrix
in the polarization basis:
\begin{equation}
  \hat{T} \repr F^{\dagger} \Omega F
  =
  \frac{i\omega}{2}
  \begin{pmatrix}
    1 & 1 \\
...
Hence we notice that time is not diagonal in the polarization basis.

Or, actually
\begin{equation}
  \hat{T} \repr \frac{\pi}{2\omega^2} F^{\dagger} \Omega F
  =
  % \frac{i\omega}{2}
  % \begin{pmatrix}
  %   1 & 1 \\
